...
## 1. THE FOURTEEN RULINGS (non-negotiable)
...
Rulings **1–5 are enforced by `tests/invariants/`** (§4). Rulings **6–14 are ALL CLOSED in code** — suite baseline **145**, invariant baseline **23/23**. **Nova STAGE 2b is LANDED: Nova is LIVE in the pipeline** (cycles once per pass, defers to authorized suspension, erupts from doctrine strain, feeds compass EAST) **and the `proposals` seam is WIRED — doctrine mutation is now STRUCTURALLY POSSIBLE**, gated only by DEE's five CMTE criteria and SAE's Self-Mutation Ceiling. Ruling 14's `echo_origin` derivation is live (STEP 1 `c6d8b7e`; STEPS 2–4 `97b6f2c`; Stage 2b pins in `tests/test_nova_stage2b.py`). **A doctrine with no survived proposal STILL FERMENTS — that remains the correct, common case; opening the path did not license making it fire.** See §8 for the closed-seam record + remaining unbuilt-module seams (TCAML, HAIL, CTL, NSC) and the open scar-fusion escalation.

**Live status is synchronized, not inferred.** When a live status fact is intentionally repeated here and in `Aurea Build/BUILD_CONTRACT.md` — including suite baselines, ruling closure, active stage, commit reference, or open-seam state — the same change must update both files in the same work unit. If they differ, **stop** and determine which reflects the latest architect ruling and repository state; never silently choose one. This is a synchronization requirement, not permission to edit either document casually.
