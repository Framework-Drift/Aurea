...
| **Nova** | **ACTIVE 2026-07-22 — Stage 1 (Ruling 12): organ + tests ONLY. Do NOT touch aurea_core, dee, compass, or any reflex file in Stage 1 — wiring is Stage 2, gated on the Stage-1 report verifying in Projects.** `nova.py` = 0 bytes. Build Engine v1 ONLY against the §1 contract: G1 provenance-raise at construction (constructor refuses an origin-less echo — the refusal IS the answer); G2 emission gate; G3 recombination-only (**no LLM in the truth-content path**); G5 suppression check at cycle entry; **timer→eligibility, never timer→Mutated** (Engine v2.0's literal `Timer → Mutation` line is OVERRULED by canon's own `Mutated → Doctrine forged / scar fused / CSA rerouted` definition — do not implement the literal line). NSC + fusion + NDR **declared-dormant in-file** (ICA dormant-trigger pattern — do not fabricate them). Single-echo proposals fill `DEE.cycle`'s `proposals` seam completely; wire compass EAST live with the build. Until built: EAST reads empty and honestly reports empty, DEE ferments — **still correct behavior; the ferment path never closes.** Fermentation magnitudes = COINED → register. |
