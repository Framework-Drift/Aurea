...
Rulings **1–5 are enforced by `tests/invariants/`** (§4). Rulings **6–13 are ALL CLOSED in code** — suite baseline **117**, invariant baseline **23/23**. **Nova STAGE 2 (wire + arbitrate) is the ACTIVE build:** live suppression read · EchoNet/DEE collapse routing · `proposals()` → `aurea_core:521` · compass EAST · consumers for the parked `scar_requests`/`csa_requests`. See §8 for the closed-seam record + remaining unbuilt-module seams (TCAML, HAIL, CTL, NSC).
