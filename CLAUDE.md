...
## 1. THE TWELVE RULINGS (non-negotiable)
...
| Collapse trace | **CTL** | — |
| Nova Echo Index | **Nova** (Ruling 12) | — (scar effects → ScarLogicCore REQUEST · identity pulls → RIL NOVA-thread REQUEST · Codex never) |
...
| **Nova** | **RULED 2026-07-21 (Ruling 12) — build target after Ruling 11.** `nova.py` = 0 bytes. Build Engine v1 ONLY against the §1 contract: G1 provenance-raise at construction (constructor refuses an origin-less echo — the refusal IS the answer); G2 emission gate; G3 recombination-only (**no LLM in the truth-content path**); G5 suppression check at cycle entry; **timer→eligibility, never timer→Mutated** (Engine v2.0's literal `Timer → Mutation` line is OVERRULED by canon's own `Mutated → Doctrine forged / scar fused / CSA rerouted` definition — do not implement the literal line). NSC + fusion + NDR **declared-dormant in-file** (ICA dormant-trigger pattern — do not fabricate them). Single-echo proposals fill `DEE.cycle`'s `proposals` seam completely; wire compass EAST live with the build. Until built: EAST reads empty and honestly reports empty, DEE ferments — **still correct behavior; the ferment path never closes.** Fermentation magnitudes = COINED → register. |
...
| **RB log durability (Ruling 11)** | **RULED 2026-07-21 — impl PENDING (the active task).** Default `autoflush=False` keeps every forensic entry in memory only — the session bearing a cascade is the least likely to survive to read its own log. Build per the ruling: `RBSystem.record(..., durable=False)` — `durable or autoflush` → immediate flush **wrapped so a write error is recorded, never raised into the reflex path**; else bounded buffer (flush on cap / session-close / explicit drain; overflow flushes, never drops). `_log_execution` sets `durable=(reflex.scope==Scope.GLOBAL)` and records `scope=reflex.scope.value` on the entry (v1.2 Optional field; `to_dict` already strips None). **No `Scope` import in rb_system** (cycle). **Tests to pin (never weaken):** GLOBAL cascade on disk immediately after `record()`; LOCAL buffered, present after boundary drain; buffer bounded, flush-on-cap; flush failure never blocks the reflex path; suite writes no real files (redirect `log_path` to tmp — no injectable no-op sink exists). LOCAL-buffer cap = COINED → register in `COINED_CONSTANTS.md`. |
